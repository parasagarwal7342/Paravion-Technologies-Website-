...
export default function Home() {
  ...
  const onSubmitQuote = (data: PrintQuoteFormInputs) => {
    console.log("E-commerce Order Submitted:", data);
    const invoice = calculatePricing();
    toast(`Order Placed! Total: ₹${invoice.total}. ID: PRV-${Math.floor(Math.random() * 900000 + 100000)}`, "success");
    reset();
  };
  ...
  return (
    ...
    >
      ...
      {/* ==========================================
          ...
          ========================================== */}
      <section id="quote-form" className="py-24 max-w-6xl mx-auto px-6 z-20 relative bg-transparent">
        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={{ once: false, margin: "-100px" }}
        ...
        >
          ...
          <h3 className="text-xs font-mono text-gold tracking-widest uppercase mb-2">E-commerce Terminal</h3>
          <h4 className="text-2xl font-bold text-white mb-6">Order Portal & Pricing Calculator</h4>
              ...
              <div className="bg-zinc-950 border border-zinc-900 rounded p-6 flex flex-col justify-between h-full relative overflow-hidden min-h-[400px]">
                    ...
                    <div className="mt-6 pt-4 border-t border-zinc-900 flex flex-col gap-2">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className={calculatePricing().deliveryFee === 0 ? "text-teal font-bold" : "text-zinc-400"}>
                          {watchedDeliveryMethod === "Self-Pickup" ? "₹0 (Pickup)" : (calculatePricing().deliveryFee === 0 ? "FREE" : `₹${calculatePricing().deliveryFee}`)}
                        </span>
                      </div>
                      
                      {watchedDeliveryMethod === "Home Delivery" && calculatePricing().subtotal < 300 && (
                        <p className="text-[9px] text-amber-500 mt-1 leading-normal">
                          * Add ₹{300 - calculatePricing().subtotal} more to qualify for FREE Home Delivery!
                        </p>
                      )}
                      {watchedDeliveryMethod === "Home Delivery" && calculatePricing().subtotal >= 300 && (
                        <p className="text-[9px] text-teal mt-1 leading-normal font-bold">
                ...
                <div className="mt-8 pt-4 border-t border-zinc-900">
                  <div className="flex justify-between items-center mb-6">
                    <span className="text-sm font-mono text-zinc-400 uppercase">Grand Total:</span>
